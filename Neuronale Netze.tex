...
Deep Convolutional Neural Networks sind besondere Formen der neuronalen Netze, die besonders in der Bilderkennung und -Klassifikation sehr häufig verwendet werden. Grund dafür ist, dass sie viel schneller und besser trainiert werden können als herkömmliche neuronale Netze. Während diese eine Genauigkeit von über 80\% nur selten erreichen, können Deep Convolutional Neural Networks Bilder meist problemlos über 95\% der Bilder korrekt klassifizieren.
...
Verwendet man ein übliches neuronales Netz für die Bildklassifikation, ist die Relevanz zweier nebeneinanderliegender Pixel genauso hoch wie die von weit entfernten Pixeln. Das folgt daraus, dass die Ausgabe jedes Neurons an jedes Neuron in der folgenden Schicht propagiert wird.

In einem Convolutional Layer ist das anders: Mithilfe eines Filters werden aus nebeneinanderliegenden Pixeln Eigenschaften erkannt. In den ersten Schichten des Netzes beschränken sich diese Eigenschaften auf Kanten und andere einfache Formen, je tiefer sich eine Schicht jedoch im Netz befindet, kann diese immer komplexere Objekte anhand von Konturen unterscheiden.

Um dies zu erreichen, wird die Convolution oder die Cross Correlation angewandt.

\subsubsection{Eindimensionale Cross Correlation}
Als \enquote{Cross Correlation} wird im Folgenden eine Operation mit zwei Vektoren $x$ und $w$ betrachtet, aus dem ein weiterer Vektor $y$ berechnet werden soll. Sie stellt eine wichtige Grundlage der Convolutional Neural Networks dar, die sogenannte \enquote{Convolutional Layer} enthalten. Außerdem ist sie eine Vereinfachung der Convolution, welche selbst nur in wenigen Machine-Learning-Frameworks implementiert ist. Der Unterschied besteht zwischen der Handhabung der Vektoren, die bei der Convolution in alle Dimensionen gespiegelt sind. Im Anwendungsfall mit einem Algorithmus ist dieser Effekt allerdings vernachlässigbar.

Unter Definition der Variablen
\begin{itemize}
\item $x$: Eingabevektor
\item $w$: Filter (vergleichbar mit Gewichtsvektor)
\item $y$: Ausgabevektor
\end{itemize}
mit den Bedingungen
\[
|x| \geq |w| \qquad |x|, \: |w| > 0
...
\end{equation}
beschrieben werden. In Worten bedeutet das, dass der Filter $w$ an den Eingaben $x$ angelegt wird. Addiert man das Produkt der einzelnen Werte $x_i$ und $w_i$, ermittelt man $y_i$. Um $y_{i+1}$ zu berechnen, muss der Filtervektor um eine Einheit verschoben werden. Ein Sinnbild ist in Abbildung \ref{fig:CrossCorrelationNoPadding} dargestellt.
...
Dabei ist offensichtlich, dass die Länge der Ausgabe stets kleiner als die Länge der Eingabe ist. Aus diesem Grund wird ein \enquote{Padding} eingeführt, welches den Eingabevektor $x$ künstlich mit Nullen erweitern soll. Zur Ermittlung des Ausgabevektors $y$ entsteht das in Abbildung \ref{fig:CrossCorrelationWithPadding} dargestellte Schema.
...
Somit wird ein Ausgabevektor $y$ erreicht, der dieselbe Länge hat wie der Eingabevektor $x$.

\subsubsection{Zweidimensionale Cross Correlation}
Bilder sind jedoch nicht eindimensional. Daher wird eine zweidimensionale Cross Correlation benötigt. Zwar lässt sie sich in Formeln durch
\begin{equation}
Y = X \odot W \quad \rightarrow \quad Y_{i, j} = \sum\limits_{k=0}^{|W|} \; \sum\limits_{l=0}^{|W_l|} X_{i+k, j+l} * W_{k, j}
\end{equation}
beschreiben, anhand eines Schemas ist sie allerdings einfacher zu verstehen.
...
Auch hier wird die Filtermatrix $W$ an die Eingabematrix $X$ angelegt und verschoben, um die Werte der Ausgabe $Y$ zu berechnen. Es ist jedoch zu beachten, dass $W$ nun nicht nur in eine, sondern in beide Dimensionen verschoben wird.

Damit keine Pixel verloren werden, wird die Eingabe $X$ ebenfalls zu einem $X^p$ mit Padding erweitert.

\subsubsection{Übersicht über Vorteile}
Ein Convolutional Layer ist also eine Schicht in einem neuronalen Netz, die nur nebeneinanderliegende Pixel mit einem immer wiederkehrenden Filter (in der Regel eine $3 \times 3$ oder eine $5 \times 5$-Matrix) betrachtet und dadurch zunächst einfache Formen, nach wenigen dieser Schichten allerdings schon komplexere Konturen erkennen kann. Dadurch wird ebenfalls die absolute Position eines zu klassifizierenden Objekts innerhalb eines Bildes egal, da lediglich die Relativität von Formen zueinander eine Rolle spielen.

Da die Schicht mit einem einheitlichen Filter mit etwa 9 bis 25 Gewichten auf Eigenschaften untersucht wird, gibt es auch wenige Variablen, die bei einem Trainingsvorgang verändert werden müssen, wodurch dieser wesentlich kürzer dauert als in einem vollständig verbundenen neuronalen Netz.
...
\node[main, fill=red!25] at (4.5, .25) {11};
